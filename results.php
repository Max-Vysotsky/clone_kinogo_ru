<?php 
  ...
  $ontherPartOfURl = '?search_query='. $search_query;
  $AllArticles = R::count( 'item', '  `title` LIKE ? ',  ['%'.$search_query.'%'] );
    require('includes/paginatorhead.php'); 
    $articles;
  if($isdesc){
      $articles = R::getAll( 'SELECT * FROM `item` WHERE `title` LIKE ? ORDER BY '.$sortingMethod[$NumOfSortingMethod].' DESC LIMIT ?,?', ['%'.$search_query.'%', $offset, $articlesOnPage] );
  }else{
      $articles = R::getAll( 'SELECT * FROM `item` WHERE `title` LIKE ? ORDER BY '.$sortingMethod[$NumOfSortingMethod].' LIMIT ?,?', ['%'.$search_query.'%', $offset, $articlesOnPage] );
  }
 // $articles = R::getAll( 'SELECT * FROM `item` WHERE `title`LIKE ? ORDER BY `id` DESC LIMIT  ?,?',['%'.$search_query.'%',$offset, $articlesOnPage] );
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="container">
          <div class="main-wrap">
            ...
            <div class="side-right gray-color">
               ...
               <div class="resutl-input-block">
                <form action="/results.php" method="get">
                    <input type="text" placeholder="поиск" class="search-input" name="search_query" value="<?php echo $search_query ?>">
                    ...
                    <article class="items__item item">
                        ...
                        <div class="item__main">
                            ...
                            <div class="side__right">
                                <p class="item__text"><?php echo $text ?></p>
