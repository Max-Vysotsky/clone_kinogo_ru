<?php require_once('db.php'); 
 $AllArticles    = R::count( 'item' );
  ...
  $articles;
  if($isdesc){
      $articles = R::getAll( 'SELECT * FROM `item` ORDER BY '.$sortingMethod[$NumOfSortingMethod].' DESC LIMIT ?,?', [ $offset, $articlesOnPage] );
  }else{
      $articles = R::getAll( 'SELECT * FROM `item` ORDER BY '.$sortingMethod[$NumOfSortingMethod].' LIMIT ?,?', [ $offset, $articlesOnPage] );
  }
?>
<!DOCTYPE html>
<html lang="en">
